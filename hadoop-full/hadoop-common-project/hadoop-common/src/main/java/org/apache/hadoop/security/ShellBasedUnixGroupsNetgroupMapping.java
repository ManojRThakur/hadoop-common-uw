...
public class ShellBasedUnixGroupsNetgroupMapping extends @OsUntrusted ShellBasedUnixGroupsMapping {
   ...
   */
  @Override
  public @OsUntrusted List<@OsTrusted String> getGroups(@OsUntrusted ShellBasedUnixGroupsNetgroupMapping this, @OsTrusted String user) throws IOException {
    // parent get unix groups
    @OsUntrusted List<@OsTrusted String> groups = new @OsUntrusted LinkedList<@OsTrusted String>(super.getGroups(user));
   ...
   */
  @Override
  public void cacheGroupsRefresh(@OsUntrusted ShellBasedUnixGroupsNetgroupMapping this) throws IOException {
    @OsUntrusted
    List<@OsTrusted String> groups = NetgroupCache.getNetgroupNames();
    NetgroupCache.clear();
   ...
   */
  @Override
  public void cacheGroupsAdd(@OsUntrusted ShellBasedUnixGroupsNetgroupMapping this, @OsUntrusted List<@OsTrusted String> groups) throws IOException {
    for(@OsTrusted String group: groups) {
      if(group.length() == 0) {
  ...
  protected @OsUntrusted List<@OsTrusted String> getUsersForNetgroup(@OsUntrusted ShellBasedUnixGroupsNetgroupMapping this, @OsTrusted String netgroup)
    throws IOException {

    @OsUntrusted List<@OsTrusted String> users = new @OsUntrusted LinkedList<@OsTrusted String>();

    // returns a string similar to this:
    // group               ( , user, ) ( domain, user1, host.com )
    @OsTrusted String usersRaw = execShellGetUserForNetgroup(netgroup);
    // get rid of spaces, makes splitting much easier
    usersRaw = (@OsTrusted String) usersRaw.replaceAll(" +", "");
    // remove netgroup name at the beginning of the string
    usersRaw = (@OsTrusted String) usersRaw.replaceFirst( netgroup.replaceFirst("@", "") + "[()]+", "");
    // split string into user infos
    @OsTrusted String @OsUntrusted [] userInfos = (@OsTrusted String @OsUntrusted []) usersRaw.split("[()]+");
    for(@OsTrusted String userInfo : userInfos) {
      // userInfo: xxx,user,yyy (xxx, yyy can be empty strings)
      // get rid of everything before first and after last comma
      @OsTrusted String user = (@OsTrusted String) userInfo.replaceFirst("[^,]*,", "");
      user = (@OsTrusted String) user.replaceFirst(",.*$", "");
      // voila! got username!
      users.add(user);
    }
   ...
   */
  @SuppressWarnings("ostrusted:cast.unsafe")
  protected @OsTrusted String execShellGetUserForNetgroup(@OsUntrusted ShellBasedUnixGroupsNetgroupMapping this, final @OsTrusted String netgroup)
      throws IOException {
    @OsTrusted String result = "";
    try {
      // shell command does not expect '@' at the begining of the group name
      result = Shell.execCommand( Shell.getUsersForNetgroupCommand( (@OsTrusted String) netgroup.substring(1)));

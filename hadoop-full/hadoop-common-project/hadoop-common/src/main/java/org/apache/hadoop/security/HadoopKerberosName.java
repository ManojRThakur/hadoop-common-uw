...
public class HadoopKerberosName extends @OsUntrusted KerberosName {

  /**
   * Create a name from the full Kerberos principal name.
   * @param name
   */
  public @OsUntrusted HadoopKerberosName(@OsUntrusted String name) {
    super(name);
  }
  /**
   ...
   * @throws IOException
   */
  public static void setConfiguration(@OsUntrusted Configuration conf) throws IOException {
    final @OsUntrusted String defaultRule;
    switch (SecurityUtil.getAuthenticationMethod(conf)) {
      case KERBEROS:
      case KERBEROS_SSL:
        try {
          KerberosUtil.getDefaultRealm();
        } catch (@OsUntrusted Exception ke) {
          throw new @OsUntrusted IllegalArgumentException("Can't get Kerberos realm", ke);
        }
        defaultRule = "DEFAULT";
        break;
      default:
  ...
  public static void main(@OsUntrusted String @OsUntrusted [] args) throws Exception {
    setConfiguration(new @OsUntrusted Configuration());
    for(@OsUntrusted String arg: args) {
      @OsUntrusted
      HadoopKerberosName name = new @OsUntrusted HadoopKerberosName(arg);

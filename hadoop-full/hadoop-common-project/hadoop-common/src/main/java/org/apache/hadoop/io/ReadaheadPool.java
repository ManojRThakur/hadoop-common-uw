...
public class ReadaheadPool {
  static final @OsUntrusted Log LOG = LogFactory.getLog(ReadaheadPool.class);
  private static final @OsUntrusted int POOL_SIZE = 4;
  private static final @OsUntrusted int MAX_POOL_SIZE = 16;
  private static final @OsUntrusted int CAPACITY = 1024;
  private final @OsUntrusted ThreadPoolExecutor pool;
  
  private static @OsUntrusted ReadaheadPool instance;

  /**
   * Return the singleton instance for the current process.
   */
  public static @OsUntrusted ReadaheadPool getInstance() {
    synchronized (ReadaheadPool.class) {
      if (instance == null && NativeIO.isAvailable()) {
        instance = new @OsUntrusted ReadaheadPool();
      }
      return instance;
    }
  }
  
  private @OsUntrusted ReadaheadPool() {
    pool = new @OsUntrusted ThreadPoolExecutor(POOL_SIZE, MAX_POOL_SIZE, 3L, TimeUnit.SECONDS,
        new @OsUntrusted ArrayBlockingQueue<@OsUntrusted Runnable>(CAPACITY));
    pool.setRejectedExecutionHandler(new ThreadPoolExecutor.@OsUntrusted DiscardOldestPolicy());
    pool.setThreadFactory(new @OsUntrusted ThreadFactoryBuilder()
      .setDaemon(true)
   ...
   */
  public @OsUntrusted ReadaheadRequest readaheadStream(
      @OsUntrusted ReadaheadPool this, @OsUntrusted
      String identifier,
      @OsUntrusted
      FileDescriptor fd,
      @OsUntrusted
   ...
   */
  public @OsUntrusted ReadaheadRequest submitReadahead(
      @OsUntrusted ReadaheadPool this, @OsUntrusted
      String identifier, @OsUntrusted FileDescriptor fd, @OsUntrusted long off, @OsUntrusted long len) {
    @OsUntrusted
    ReadaheadRequestImpl req = new @OsUntrusted ReadaheadRequestImpl(
  ...
  public interface ReadaheadRequest {
     ...
     * been fulfilled.
     */
    public void cancel(ReadaheadPool.@OsUntrusted ReadaheadRequest this);
    
    /**
     * @return the requested offset
     */
    public @OsUntrusted long getOffset(ReadaheadPool.@OsUntrusted ReadaheadRequest this);

    /**
     * @return the requested length
     */
    public @OsUntrusted long getLength(ReadaheadPool.@OsUntrusted ReadaheadRequest this);
  }
  
  private static class ReadaheadRequestImpl implements @OsUntrusted Runnable, @OsUntrusted ReadaheadRequest {
    private final @OsUntrusted String identifier;
    private final @OsUntrusted FileDescriptor fd;
    private final @OsUntrusted long off, len;
    private volatile @OsUntrusted boolean canceled = false;
    
    private @OsUntrusted ReadaheadRequestImpl(@OsUntrusted String identifier, @OsUntrusted FileDescriptor fd, @OsUntrusted long off, @OsUntrusted long len) {
    ...
    @Override
    public void run(ReadaheadPool.@OsUntrusted ReadaheadRequestImpl this) {
      if (canceled) return;
    ...
    @Override
    public void cancel(ReadaheadPool.@OsUntrusted ReadaheadRequestImpl this) {
      canceled = true;
    ...
    @Override
    public @OsUntrusted long getOffset(ReadaheadPool.@OsUntrusted ReadaheadRequestImpl this) {
      return off;
    }

    @Override
    public @OsUntrusted long getLength(ReadaheadPool.@OsUntrusted ReadaheadRequestImpl this) {
      return len;
    }

    @Override
    public @OsUntrusted String toString(ReadaheadPool.@OsUntrusted ReadaheadRequestImpl this) {

...
class Stat extends @OsUntrusted FsCommand {
  public static void registerCommands(@OsUntrusted CommandFactory factory) {
    factory.addClass(Stat.class, "-stat");
  }

  public static final @OsUntrusted String NAME = "stat";
  public static final @OsUntrusted String USAGE = "[format] <path> ...";
  ...
  protected static final @OsUntrusted SimpleDateFormat timeFmt;
  static {
    timeFmt = new @OsUntrusted SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
    timeFmt.setTimeZone(TimeZone.getTimeZone("UTC"));
  }

  // default format string
  protected @OsUntrusted String format = "%y";

  @Override
  protected void processOptions(@OsUntrusted Stat this, @OsUntrusted LinkedList<@OsUntrusted String> args) throws IOException {
    @OsUntrusted
    CommandFormat cf = new @OsUntrusted CommandFormat(1, Integer.MAX_VALUE, "R");
    cf.parse(args);
  ...
  @Override
  protected void processPath(@OsUntrusted Stat this, @OsUntrusted PathData item) throws IOException {
    @OsUntrusted
    ...
    @OsUntrusted
    char @OsUntrusted [] fmt = format.toCharArray();
    for (@OsUntrusted int i = 0; i < fmt.length; ++i) {
      if (fmt[i] != '%') {
        buf.append(fmt[i]);
      } else {
        ...
        switch (fmt[++i]) {
          ...
          case 'u':
            ...
            break;
          case 'y':
            buf.append(timeFmt.format(new @OsUntrusted Date(stat.getModificationTime())));
            break;
          case 'Y':

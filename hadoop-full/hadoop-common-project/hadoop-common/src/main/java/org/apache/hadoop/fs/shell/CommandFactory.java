...
public class CommandFactory extends @OsUntrusted Configured implements @OsUntrusted Configurable {
  private @OsUntrusted Map<@OsUntrusted String, @OsUntrusted Class<@OsUntrusted ? extends @OsUntrusted Command>> classMap =
    new @OsUntrusted HashMap<@OsUntrusted String, @OsUntrusted Class<@OsUntrusted ? extends @OsUntrusted Command>>();

  private @OsUntrusted Map<@OsUntrusted String, @OsUntrusted Command> objectMap =
    new @OsUntrusted HashMap<@OsUntrusted String, @OsUntrusted Command>();

  /** Factory constructor for commands */
  public @OsUntrusted CommandFactory() {
    this(null);
  }
   ...
   */
  public @OsUntrusted CommandFactory(@OsUntrusted Configuration conf) {
    super(conf);
  }
   ...
   */
  public void registerCommands(@OsUntrusted CommandFactory this, @OsUntrusted Class<@OsUntrusted ? extends java.lang.@OsUntrusted Object> registrarClass) {
    try {
      registrarClass.getMethod(
          "registerCommands", CommandFactory.class
      ).invoke(null, this);
    } catch (@OsUntrusted Exception e) {
      throw new @OsUntrusted RuntimeException(StringUtils.stringifyException(e));
    }
  }
   ...
   */
  public void addClass(@OsUntrusted CommandFactory this, @OsUntrusted Class<@OsUntrusted ? extends @OsUntrusted Command> cmdClass, @OsUntrusted String @OsUntrusted ... names) {
    for (@OsUntrusted String name : names) classMap.put(name, cmdClass);
  }
   ...
   * @param names one or more command names that will invoke this class
   */
  public void addObject(@OsUntrusted CommandFactory this, @OsUntrusted Command cmdObject, @OsUntrusted String @OsUntrusted ... names) {
   ...
   */
  public @OsUntrusted Command getInstance(@OsUntrusted CommandFactory this, @OsUntrusted String cmd) {
   ...
   */
  public @OsUntrusted Command getInstance(@OsUntrusted CommandFactory this, @OsUntrusted String cmdName, @OsUntrusted Configuration conf) {
    if (conf == null) throw new @OsUntrusted NullPointerException("configuration is null");
    
    @OsUntrusted
    Command instance = objectMap.get(cmdName);
    if (instance == null) {
      @OsUntrusted
      Class<@OsUntrusted ? extends @OsUntrusted Command> cmdClass = classMap.get(cmdName);
      if (cmdClass != null) {
   ...
   */
  public @OsUntrusted String @OsUntrusted [] getNames(@OsUntrusted CommandFactory this) {
    @OsUntrusted
    String @OsUntrusted [] names = classMap.keySet().toArray(new @OsUntrusted String @OsUntrusted [0]);
    Arrays.sort(names);
    return names;
  }
}

...
public class NetworkTopologyWithNodeGroup extends @OsUntrusted NetworkTopology {

  public final static @OsUntrusted String DEFAULT_NODEGROUP = "/default-nodegroup";

  public @OsUntrusted NetworkTopologyWithNodeGroup() {
    clusterMap = new @OsUntrusted InnerNodeWithNodeGroup(InnerNode.ROOT);
  }

  @Override
  protected @OsUntrusted Node getNodeForNetworkLocation(@OsUntrusted NetworkTopologyWithNodeGroup this, @OsUntrusted Node node) {
    ...
    if (nodeGroup == null) {
      nodeGroup = new @OsUntrusted InnerNodeWithNodeGroup(node.getNetworkLocation());
    }
    return getNode(nodeGroup.getNetworkLocation());
  }

  @Override
  public @OsUntrusted String getRack(@OsUntrusted NetworkTopologyWithNodeGroup this, @OsUntrusted String loc) {
    netlock.readLock().lock();
    try {
      loc = InnerNode.normalize(loc);
      @OsUntrusted
      Node locNode = getNode(loc);
      if (locNode instanceof @OsUntrusted InnerNodeWithNodeGroup) {
        @OsUntrusted
        InnerNodeWithNodeGroup node = (@OsUntrusted InnerNodeWithNodeGroup) locNode;
        if (node.isRack()) {
          return loc;
   ...
   */
  public @OsUntrusted String getNodeGroup(@OsUntrusted NetworkTopologyWithNodeGroup this, @OsUntrusted String loc) {
    netlock.readLock().lock();
    try {
      loc = InnerNode.normalize(loc);
      @OsUntrusted
  ...
  @Override
  public @OsUntrusted boolean isOnSameRack( @OsUntrusted NetworkTopologyWithNodeGroup this, @OsUntrusted Node node1,  @OsUntrusted Node node2) {
   ...
   */
  @Override
  public @OsUntrusted boolean isOnSameNodeGroup(@OsUntrusted NetworkTopologyWithNodeGroup this, @OsUntrusted Node node1, @OsUntrusted Node node2) {
    if (node1 == null || node2 == null) {
      return false;
    }
    netlock.readLock().lock();
   ...
   */
  @Override
  public @OsUntrusted boolean isNodeGroupAware(@OsUntrusted NetworkTopologyWithNodeGroup this) {
    return true;
  }
   ...
   */
  @Override
  public void add(@OsUntrusted NetworkTopologyWithNodeGroup this, @OsUntrusted Node node) {
    if (node==null) return;
    ...
    try {
      ...
      if (rack != null && 
          (!(rack instanceof @OsUntrusted InnerNode) || rack.getParent() == null)) {
   ...
   */
  @Override
  public void remove(@OsUntrusted NetworkTopologyWithNodeGroup this, @OsUntrusted Node node) {
    if (node==null) return;
    if( node instanceof @OsUntrusted InnerNode ) {
      throw new @OsUntrusted IllegalArgumentException(
    ...
    try {
      if (clusterMap.remove(node)) {
        ...
        Node nodeGroup = getNode(node.getNetworkLocation());
        if (nodeGroup == null) {
          nodeGroup = new @OsUntrusted InnerNode(node.getNetworkLocation());
        }
        @OsUntrusted
        InnerNode rack = (@OsUntrusted InnerNode)getNode(nodeGroup.getNetworkLocation());
        if (rack == null) {
          numOfRacks--;
        }
      }
   ...
   */
  @Override
  public void pseudoSortByDistance( @OsUntrusted NetworkTopologyWithNodeGroup this, @OsUntrusted Node reader, @OsUntrusted Node @OsUntrusted [] nodes ) {
    ...
    if (reader != null && !this.contains(reader)) {
      ...
      if (nodeGroup != null && nodeGroup instanceof @OsUntrusted InnerNode) {
        @OsUntrusted
        InnerNode parentNode = (@OsUntrusted InnerNode) nodeGroup;
   ...
   */
  static class InnerNodeWithNodeGroup extends @OsUntrusted InnerNode {
    public @OsUntrusted InnerNodeWithNodeGroup(@OsUntrusted String name, @OsUntrusted String location, 
        @OsUntrusted
        InnerNode parent, @OsUntrusted int level) {
    ...
    public @OsUntrusted InnerNodeWithNodeGroup(@OsUntrusted String path) {
      super(path);
    }

    @Override
    @OsUntrusted
    boolean isRack(NetworkTopologyWithNodeGroup.@OsUntrusted InnerNodeWithNodeGroup this) {
      // it is node group
      ...
      if (firstChild instanceof @OsUntrusted InnerNode) {
        @OsUntrusted
        Node firstGrandChild = (((@OsUntrusted InnerNode) firstChild).children).get(0);
        if (firstGrandChild instanceof @OsUntrusted InnerNode) {
          // it is datacenter
          return false;
        } else {
          return true;
     ...
     */
    @OsUntrusted
    boolean isNodeGroup(NetworkTopologyWithNodeGroup.@OsUntrusted InnerNodeWithNodeGroup this) {
      if (children.isEmpty()) {
        return true;
      }
      @OsUntrusted
    ...
    @Override
    protected @OsUntrusted boolean isLeafParent(NetworkTopologyWithNodeGroup.@OsUntrusted InnerNodeWithNodeGroup this) {
      return isNodeGroup();
    }

    @Override
    protected @OsUntrusted InnerNode createParentNode(NetworkTopologyWithNodeGroup.@OsUntrusted InnerNodeWithNodeGroup this, @OsUntrusted String parentName) {
      return new @OsUntrusted InnerNodeWithNodeGroup(parentName, getPath(this), this,
          this.getLevel() + 1);
    }

    @Override
    protected @OsUntrusted boolean areChildrenLeaves(NetworkTopologyWithNodeGroup.@OsUntrusted InnerNodeWithNodeGroup this) {
      return isNodeGroup();
    }
  }
}

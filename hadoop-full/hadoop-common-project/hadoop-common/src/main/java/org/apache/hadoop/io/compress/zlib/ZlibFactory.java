...
public class ZlibFactory {
   ...
   */
  public static @OsUntrusted boolean isNativeZlibLoaded(@OsUntrusted Configuration conf) {
  ...
  getZlibCompressorType(@OsUntrusted Configuration conf) {
   ...
   * @return the appropriate implementation of the zlib compressor.
   */
  public static @OsUntrusted Compressor getZlibCompressor(@OsUntrusted Configuration conf) {
    return (isNativeZlibLoaded(conf)) ? 
      new @OsUntrusted ZlibCompressor(conf) :
      new @OsUntrusted BuiltInZlibDeflater(ZlibFactory.getCompressionLevel(conf).compressionLevel());
  }
  ...
  getZlibDecompressorType(@OsUntrusted Configuration conf) {
    return (isNativeZlibLoaded(conf)) ? 
   ...
   * @return the appropriate implementation of the zlib decompressor.
   */
  public static @OsUntrusted Decompressor getZlibDecompressor(@OsUntrusted Configuration conf) {
    return (isNativeZlibLoaded(conf)) ? 
      new @OsUntrusted ZlibDecompressor() : new @OsUntrusted BuiltInZlibInflater(); 
  }

  public static void setCompressionStrategy(@OsUntrusted Configuration conf,
      @OsTrusted CompressionStrategy strategy) {
  ...
  public static @OsUntrusted CompressionStrategy getCompressionStrategy(@OsUntrusted Configuration conf) {
    return conf.getEnum("zlib.compress.strategy", CompressionStrategy.DEFAULT_STRATEGY);
  }

  public static void setCompressionLevel(Configuration conf, @OsTrusted CompressionLevel level) {
    conf.setEnum("zlib.compress.level", level);
  }

  @SuppressWarnings("ostrusted:type.argument") // The default enum should be ostrusted but the annotation does not seem to work
  public static @OsUntrusted CompressionLevel getCompressionLevel(@OsUntrusted Configuration conf) {

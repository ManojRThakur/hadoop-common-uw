 ...
 */
@DoNotPool
public class BuiltInGzipDecompressor implements @OsUntrusted Decompressor {
  private static final @OsUntrusted int GZIP_MAGIC_ID = 0x8b1f;  // if read as LE short int
  private static final @OsUntrusted int GZIP_DEFLATE_METHOD = 8;
  private static final @OsUntrusted int GZIP_FLAGBIT_HEADER_CRC  = 0x02;
  private static final @OsUntrusted int GZIP_FLAGBIT_EXTRA_FIELD = 0x04;
  private static final @OsUntrusted int GZIP_FLAGBIT_FILENAME    = 0x08;
  private static final @OsUntrusted int GZIP_FLAGBIT_COMMENT     = 0x10;
  private static final @OsUntrusted int GZIP_FLAGBITS_RESERVED   = 0xe0;

  // 'true' (nowrap) => Inflater will handle raw deflate stream only
  private @OsUntrusted Inflater inflater = new @OsUntrusted Inflater(true);
  ...
  private @OsUntrusted byte @OsUntrusted [] localBuf = new @OsUntrusted byte @OsUntrusted [256];
  private @OsUntrusted int localBufOff = 0;

  private @OsUntrusted int headerBytesRead = 0;
  private @OsUntrusted int trailerBytesRead = 0;
  private @OsUntrusted int numExtraFieldBytesRemaining = -1;
  private @OsUntrusted PureJavaCrc32 crc = new @OsUntrusted PureJavaCrc32();
  private @OsUntrusted boolean hasExtraField = false;
  private @OsUntrusted boolean hasFilename = false;
  private @OsUntrusted boolean hasComment = false;
  private @OsUntrusted boolean hasHeaderCRC = false;

  private @OsUntrusted GzipStateLabel state;
...
@OsUntrusted  FINISHED;
  }

  /**
   * Creates a new (pure Java) gzip decompressor.
   */
  public @OsUntrusted BuiltInGzipDecompressor() {
  ...
  @Override
  public synchronized @OsUntrusted boolean needsInput(@OsUntrusted BuiltInGzipDecompressor this) {
   ...
   */
  @Override
  public synchronized void setInput(@OsUntrusted BuiltInGzipDecompressor this, @OsUntrusted byte @OsUntrusted [] b, @OsUntrusted int off, @OsUntrusted int len) {
    if (b == null) {
   ...
   */
  @Override
  public synchronized @OsUntrusted int decompress(@OsUntrusted BuiltInGzipDecompressor this, @OsUntrusted byte @OsUntrusted [] b, @OsUntrusted int off, @OsUntrusted int len)
  throws IOException {
    @OsUntrusted
    int numAvailBytes = 0;
   ...
   */
  private void executeHeaderState(@OsUntrusted BuiltInGzipDecompressor this) throws IOException {
      ...
      if (hasHeaderCRC) {
        ...
        if (localBufOff >= 2) {
          ...
          if (headerCRC != (crc.getValue() & 0xffff)) {
            throw new @OsUntrusted IOException("gzip header CRC failure");
          }
          localBufOff = 0;
          crc.reset();
   ...
   */
  private void executeTrailerState(@OsUntrusted BuiltInGzipDecompressor this) throws IOException {
    ...
    if (state == GzipStateLabel.TRAILER_CRC) {
      ...
      if (localBufOff >= 4) {
        ...
        if (streamCRC != crc.getValue()) {
          throw new @OsUntrusted IOException("gzip stream CRC failure");
        }
        localBufOff = 0;
        crc.reset();
   ...
   */
  public synchronized @OsUntrusted long getBytesRead(@OsUntrusted BuiltInGzipDecompressor this) {
   ...
   */
  @Override
  public synchronized @OsUntrusted int getRemaining(@OsUntrusted BuiltInGzipDecompressor this) {
    return userBufLen;
  }

  @Override
  public synchronized @OsUntrusted boolean needsDictionary(@OsUntrusted BuiltInGzipDecompressor this) {
    return inflater.needsDictionary();
  }

  @Override
  public synchronized void setDictionary(@OsUntrusted BuiltInGzipDecompressor this, @OsUntrusted byte @OsUntrusted [] b, @OsUntrusted int off, @OsUntrusted int len) {
   ...
   */
  @Override
  public synchronized @OsUntrusted boolean finished(@OsUntrusted BuiltInGzipDecompressor this) {
    return (state == GzipStateLabel.FINISHED);
  }
   ...
   */
  @Override
  public synchronized void reset(@OsUntrusted BuiltInGzipDecompressor this) {
  ...
  @Override
  public synchronized void end(@OsUntrusted BuiltInGzipDecompressor this) {
    inflater.end();
  }
   ...
   *  [bit 5   encrypted]
   */
  private void processBasicHeader(@OsUntrusted BuiltInGzipDecompressor this) throws IOException {
    if (readUShortLE(localBuf, 0) != GZIP_MAGIC_ID) {
      throw new @OsUntrusted IOException("not a gzip file");
    }
    if (readUByte(localBuf, 2) != GZIP_DEFLATE_METHOD) {
      throw new @OsUntrusted IOException("gzip data not compressed with deflate method");
    }
    @OsUntrusted
    int flg = readUByte(localBuf, 3);
    if ((flg & GZIP_FLAGBITS_RESERVED) != 0) {
      throw new @OsUntrusted IOException("unknown gzip format (reserved flagbits set)");
    }
  ...
  private void checkAndCopyBytesToLocal(@OsUntrusted BuiltInGzipDecompressor this, @OsUntrusted int len) {
  ...
  private void checkAndSkipBytes(@OsUntrusted BuiltInGzipDecompressor this, @OsUntrusted int len) {
    crc.update(userBuf, userBufOff, len);
    userBufOff += len;
    userBufLen -= len;
    headerBytesRead += len;
  ...
  private @OsUntrusted boolean checkAndSkipBytesUntilNull(@OsUntrusted BuiltInGzipDecompressor this) {
    @OsUntrusted
  ...
  private void copyBytesToLocal(@OsUntrusted BuiltInGzipDecompressor this, @OsUntrusted int len) {
    System.arraycopy(userBuf, userBufOff, localBuf, localBufOff, len);
    localBufOff += len;
    userBufOff += len;
    userBufLen -= len;
  ...
  private @OsUntrusted int readUByte(@OsUntrusted BuiltInGzipDecompressor this, @OsUntrusted byte @OsUntrusted [] b, @OsUntrusted int off) {
    return ((@OsUntrusted int)b[off] & 0xff);
  }

  // caller is responsible for not overrunning buffer
  private @OsUntrusted int readUShortLE(@OsUntrusted BuiltInGzipDecompressor this, @OsUntrusted byte @OsUntrusted [] b, @OsUntrusted int off) {
    return ((((b[off+1] & 0xff) << 8) |
             ((b[off]   & 0xff)     )) & 0xffff);
  }

  // caller is responsible for not overrunning buffer
  private @OsUntrusted long readUIntLE(@OsUntrusted BuiltInGzipDecompressor this, @OsUntrusted byte @OsUntrusted [] b, @OsUntrusted int off) {
    return ((((@OsUntrusted long)(b[off+3] & 0xff) << 24) |
             ((@OsUntrusted long)(b[off+2] & 0xff) << 16) |
             ((@OsUntrusted long)(b[off+1] & 0xff) <<  8) |
             ((@OsUntrusted long)(b[off]   & 0xff)      )) & 0xffffffffL);
  }

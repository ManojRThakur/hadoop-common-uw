...
public class JRecord extends @OsUntrusted JCompType {
  
  class JavaRecord extends @OsUntrusted JavaCompType {
    
    private @OsUntrusted String fullName;
    private @OsUntrusted String name;
    private @OsUntrusted String module;
    private @OsUntrusted ArrayList<@OsUntrusted JField<@OsUntrusted JavaType>> fields =
      new @OsUntrusted ArrayList<@OsUntrusted JField<@OsUntrusted JavaType>>();
    
    @OsUntrusted
    JavaRecord(@OsUntrusted String name, @OsUntrusted ArrayList<@OsUntrusted JField<@OsUntrusted JType>> flist) {
      ...
      for (@OsUntrusted Iterator<@OsUntrusted JField<@OsUntrusted JType>> iter = flist.iterator(); iter.hasNext();) {
        @OsUntrusted
        JField<@OsUntrusted JType> f = iter.next();
        fields.add(new @OsUntrusted JField<@OsUntrusted JavaType>(f.getName(), f.getType().getJavaType()));
      }
    }
    
    @Override
    @OsUntrusted
    String getTypeIDObjectString(@OsUntrusted JRecord.JavaRecord this) {
      return "new org.apache.hadoop.record.meta.StructTypeID(" + 
      fullName + ".getTypeInfo())";
    }

    @Override
    void genSetRTIFilter(@OsUntrusted JRecord.JavaRecord this, @OsUntrusted CodeBuffer cb, @OsUntrusted Map<@OsUntrusted String, @OsUntrusted Integer> nestedStructMap) {
    ...
    void genSetupRtiFields(@OsUntrusted JRecord.JavaRecord this, @OsUntrusted CodeBuffer cb) {
    ...
    @Override
    void genReadMethod(@OsUntrusted JRecord.JavaRecord this, @OsUntrusted CodeBuffer cb, @OsUntrusted String fname, @OsUntrusted String tag, @OsUntrusted boolean decl) {
      if (decl) {
    ...
    @Override
    void genWriteMethod(@OsUntrusted JRecord.JavaRecord this, @OsUntrusted CodeBuffer cb, @OsUntrusted String fname, @OsUntrusted String tag) {
      cb.append(fname+".serialize(" + Consts.RECORD_OUTPUT + ",\""+tag+"\");\n");
    }
    
    @Override
    void genSlurpBytes(@OsUntrusted JRecord.JavaRecord this, @OsUntrusted CodeBuffer cb, @OsUntrusted String b, @OsUntrusted String s, @OsUntrusted String l) {
      cb.append("{\n");
    ...
    @Override
    void genCompareBytes(@OsUntrusted JRecord.JavaRecord this, @OsUntrusted CodeBuffer cb) {
      cb.append("{\n");
    ...
    void genCode(@OsUntrusted JRecord.JavaRecord this, @OsUntrusted String destDir, @OsUntrusted ArrayList<@OsUntrusted String> options) throws IOException {
      @OsUntrusted
      String pkg = module;
      @OsUntrusted
      String pkgpath = pkg.replaceAll("\\.", "/");
      @OsUntrusted
      File pkgdir = new @OsUntrusted File(destDir, pkgpath);

      final @OsUntrusted File jfile = new @OsUntrusted File(pkgdir, name+".java");
      if (!pkgdir.exists()) {
        // create the pkg directory
        @OsUntrusted
        boolean ret = pkgdir.mkdirs();
        if (!ret) {
          throw new @OsUntrusted IOException("Cannnot create directory: "+pkgpath);
        }
      } else if (!pkgdir.isDirectory()) {
        // not a directory
        throw new @OsUntrusted IOException(pkgpath+" is not a directory.");
      }

      @OsUntrusted
      CodeBuffer cb = new @OsUntrusted CodeBuffer();
      ...
      cb.append(Consts.RTI_VAR + " = " +
          ...
          name + "\");\n");
      for (@OsUntrusted Iterator<@OsUntrusted JField<@OsUntrusted JavaType>> i = fields.iterator(); i.hasNext();) {
        @OsUntrusted
        JField<@OsUntrusted JavaType> jf = i.next();
        @OsUntrusted
        String name = jf.getName();
        @OsUntrusted
        JavaType type = jf.getType();
      ...
      @OsUntrusted
      int fIdx = 0;
      for (@OsUntrusted Iterator<@OsUntrusted JField<@OsUntrusted JavaType>> i = fields.iterator(); i.hasNext(); fIdx++) {
        @OsUntrusted
        JField<@OsUntrusted JavaType> jf = i.next();
        @OsUntrusted
        String name = jf.getName();
      ...
      @OsUntrusted
      Map<@OsUntrusted String, @OsUntrusted Integer> nestedStructMap = new @OsUntrusted HashMap<@OsUntrusted String, @OsUntrusted Integer>();
      for (@OsUntrusted JField<@OsUntrusted JavaType> jf : fields) {
        @OsUntrusted
        JavaType type = jf.getType();
      ...
      @OsUntrusted
      FileWriter jj = new @OsUntrusted FileWriter(jfile);
      try {
  ...
  class CppRecord extends @OsUntrusted CppCompType {
    
    private @OsUntrusted String fullName;
    private @OsUntrusted String name;
    private @OsUntrusted String module;
    private @OsUntrusted ArrayList<@OsUntrusted JField<@OsUntrusted CppType>> fields = 
      new @OsUntrusted ArrayList<@OsUntrusted JField<@OsUntrusted CppType>>();
    
    @OsUntrusted
    CppRecord(@OsUntrusted String name, @OsUntrusted ArrayList<@OsUntrusted JField<@OsUntrusted JType>> flist) {
      ...
      for (@OsUntrusted Iterator<@OsUntrusted JField<@OsUntrusted JType>> iter = flist.iterator(); iter.hasNext();) {
        @OsUntrusted
        JField<@OsUntrusted JType> f = iter.next();
        fields.add(new @OsUntrusted JField<@OsUntrusted CppType>(f.getName(), f.getType().getCppType()));
      }
    }
    
    @Override
    @OsUntrusted
    String getTypeIDObjectString(@OsUntrusted JRecord.CppRecord this) {
      return "new ::hadoop::StructTypeID(" + 
      fullName + "::getTypeInfo().getFieldTypeInfos())";
    }

    @OsUntrusted
    String genDecl(@OsUntrusted JRecord.CppRecord this, @OsUntrusted String fname) {
      return "  "+name+" "+fname+";\n";
    }
    
    @Override
    void genSetRTIFilter(@OsUntrusted JRecord.CppRecord this, @OsUntrusted CodeBuffer cb) {
      // we set the RTI filter here
      cb.append(fullName + "::setTypeFilter(rti.getNestedStructTypeInfo(\""+
          name + "\"));\n");
    }

    void genSetupRTIFields(@OsUntrusted JRecord.CppRecord this, @OsUntrusted CodeBuffer cb) {
    ...
    void genCode(@OsUntrusted JRecord.CppRecord this, @OsUntrusted FileWriter hh, @OsUntrusted FileWriter cc, @OsUntrusted ArrayList<@OsUntrusted String> options)
      throws IOException {
      @OsUntrusted
      CodeBuffer hb = new @OsUntrusted CodeBuffer();
      
      @OsUntrusted
      String @OsUntrusted [] ns = module.split("::");
      for (@OsUntrusted int i = 0; i < ns.length; i++) {
      ...
      for (@OsUntrusted Iterator<@OsUntrusted JField<@OsUntrusted CppType>> i = fields.iterator(); i.hasNext();) {
        @OsUntrusted
        JField<@OsUntrusted CppType> jf = i.next();
        @OsUntrusted
        String name = jf.getName();
        @OsUntrusted
      ...
      for (@OsUntrusted int i=ns.length-1; i>=0; i--) {
      ...
      for (@OsUntrusted Iterator<@OsUntrusted JField<@OsUntrusted CppType>> i = fields.iterator(); i.hasNext();) {
        ...
        @OsUntrusted
        CppType type = jf.getType();
        if (type instanceof @OsUntrusted JBuffer.@OsUntrusted CppBuffer) {
  ...
  class CRecord extends @OsUntrusted CCompType {
    
  }
  
  private @OsUntrusted String signature;
  
  /**
   * Creates a new instance of JRecord
   */
  public @OsUntrusted JRecord(@OsUntrusted String name, @OsUntrusted ArrayList<@OsUntrusted JField<@OsUntrusted JType>> flist) {
    setJavaType(new @OsUntrusted JavaRecord(name, flist));
    setCppType(new @OsUntrusted CppRecord(name, flist));
    setCType(new @OsUntrusted CRecord());
    ...
    for (@OsUntrusted Iterator<@OsUntrusted JField<@OsUntrusted JType>> i = flist.iterator(); i.hasNext();) {
      @OsUntrusted
  ...
  @Override
  @OsUntrusted
  String getSignature(@OsUntrusted JRecord this) {
    return signature;
  }
  
  void genCppCode(@OsUntrusted JRecord this, @OsUntrusted FileWriter hh, @OsUntrusted FileWriter cc, @OsUntrusted ArrayList<@OsUntrusted String> options)
    throws IOException {
    ((@OsUntrusted CppRecord)getCppType()).genCode(hh, cc, options);
  }
  
  void genJavaCode(@OsUntrusted JRecord this, @OsUntrusted String destDir, @OsUntrusted ArrayList<@OsUntrusted String> options)
    throws IOException {
    ((@OsUntrusted JavaRecord)getJavaType()).genCode(destDir, options);
  }
}

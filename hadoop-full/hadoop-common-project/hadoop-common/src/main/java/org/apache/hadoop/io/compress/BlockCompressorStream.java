...
public class BlockCompressorStream extends @OsUntrusted CompressorStream {

  // The 'maximum' size of input data to be compressed, to account
  // for the overhead of the compression algorithm.
  private final @OsUntrusted int MAX_INPUT_SIZE;
   ...
   */
  public @OsUntrusted BlockCompressorStream(@OsUntrusted OutputStream out, @OsUntrusted Compressor compressor, 
                               @OsUntrusted
                               int bufferSize, @OsUntrusted int compressionOverhead) {
   ...
   */
  @Override
  public void write(@OsUntrusted BlockCompressorStream this, @OsUntrusted byte @OsUntrusted [] b, @OsUntrusted int off, @OsUntrusted int len) throws IOException {
    // Sanity checks
    ...
    if (!compressor.needsInput()) {
      ...
      rawWriteInt((@OsUntrusted int)compressor.getBytesRead());
      do {
        compress();
  ...
  @Override
  public void finish(@OsUntrusted BlockCompressorStream this) throws IOException {
    if (!compressor.finished()) {
      rawWriteInt((@OsUntrusted int)compressor.getBytesRead());
      compressor.finish();
      while (!compressor.finished()) {
  ...
  @Override
  protected void compress(@OsUntrusted BlockCompressorStream this) throws IOException {
    @OsUntrusted
  ...
  private void rawWriteInt(@OsUntrusted BlockCompressorStream this, @OsUntrusted int v) throws IOException {
    out.write((v >>> 24) & 0xFF);
    out.write((v >>> 16) & 0xFF);
    out.write((v >>>  8) & 0xFF);
    out.write((v >>>  0) & 0xFF);

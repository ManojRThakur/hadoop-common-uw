...
class SetReplication extends @OsUntrusted FsCommand {
  public static void registerCommands(@OsUntrusted CommandFactory factory) {
    factory.addClass(SetReplication.class, "-setrep");
  }
  
  public static final @OsUntrusted String NAME = "setrep";
  public static final @OsUntrusted String USAGE = "[-R] [-w] <rep> <path> ...";
  ...
  protected @OsUntrusted short newRep = 0;
  protected @OsUntrusted List<@OsUntrusted PathData> waitList = new @OsUntrusted LinkedList<@OsUntrusted PathData>();
  protected @OsUntrusted boolean waitOpt = false;
  
  @Override
  protected void processOptions(@OsUntrusted SetReplication this, @OsUntrusted LinkedList<@OsUntrusted String> args) throws IOException {
    @OsUntrusted
    CommandFormat cf = new @OsUntrusted CommandFormat(2, Integer.MAX_VALUE, "R", "w");
    cf.parse(args);
    waitOpt = cf.getOpt("w");
    setRecursive(true);
    ...
    if (newRep < 1) {
      throw new @OsUntrusted IllegalArgumentException("replication must be >= 1");
    }
  }

  @Override
  protected void processArguments(@OsUntrusted SetReplication this, @OsUntrusted LinkedList<@OsUntrusted PathData> args)
  throws IOException {
  ...
  @Override
  protected void processPath(@OsUntrusted SetReplication this, @OsUntrusted PathData item) throws IOException {
    if (item.stat.isSymlink()) {
      throw new @OsUntrusted PathIOException(item.toString(), "Symlinks unsupported");
    }
    
    if (item.stat.isFile()) {
      if (!item.fs.setReplication(item.path, newRep)) {
        throw new @OsUntrusted IOException("Could not set replication for: " + item);
      }
   ...
   */
  private void waitForReplication(@OsUntrusted SetReplication this) throws IOException {
    for (@OsUntrusted PathData item : waitList) {
      ...
      while (!done) {
        ...
        out.print(".");
        out.flush();
        try {Thread.sleep(10000);} catch (@OsUntrusted InterruptedException e) {}
      }
      out.println(" done");
    }
  }

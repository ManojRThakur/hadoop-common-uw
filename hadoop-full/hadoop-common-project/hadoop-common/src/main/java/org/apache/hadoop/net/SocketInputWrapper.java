...
public class SocketInputWrapper extends @OsUntrusted FilterInputStream {
  private final @OsUntrusted Socket socket;
  private final @OsUntrusted boolean hasChannel;

  @OsUntrusted
  SocketInputWrapper(@OsUntrusted Socket s, @OsUntrusted InputStream is) {
    super(is);
    this.socket = s;
    this.hasChannel = s.getChannel() != null;
    if (hasChannel) {
      Preconditions.checkArgument(is instanceof @OsUntrusted SocketInputStream,
   ...
   */
  public void setTimeout(@OsUntrusted SocketInputWrapper this, @OsUntrusted long timeoutMs) throws SocketException {
    if (hasChannel) {
      ((@OsUntrusted SocketInputStream)in).setTimeout(timeoutMs);
    } else {
      socket.setSoTimeout((@OsUntrusted int)timeoutMs);
    }
  }
   ...
   */
  public @OsUntrusted ReadableByteChannel getReadableByteChannel(@OsUntrusted SocketInputWrapper this) {
    Preconditions.checkState(hasChannel,
        "Socket %s does not have a channel",
        this.socket);
    return (@OsUntrusted SocketInputStream)in;
  }
}
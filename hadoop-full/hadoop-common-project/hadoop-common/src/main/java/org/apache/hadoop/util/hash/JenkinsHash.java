...
public class JenkinsHash extends @OsUntrusted Hash {
  private static @OsUntrusted long INT_MASK  = 0x00000000ffffffffL;
  private static @OsUntrusted long BYTE_MASK = 0x00000000000000ffL;
  
  private static @OsUntrusted JenkinsHash _instance = new @OsUntrusted JenkinsHash();
  
  public static @OsUntrusted Hash getInstance() {
    return _instance;
  }

  private static @OsUntrusted long rot(@OsUntrusted long val, @OsUntrusted int pos) {
    return ((Integer.rotateLeft(
        (@OsUntrusted int)(val & INT_MASK), pos)) & INT_MASK);
  }
  ...
  */
  ...
  public @OsUntrusted int hash(@OsUntrusted JenkinsHash this, @OsUntrusted byte @OsUntrusted [] key, @OsUntrusted int nbytes, @OsUntrusted int initval) {
    @OsUntrusted
    int length = nbytes;
    ...
    case  1:
      ...
      break;
    case  0:
      return (@OsUntrusted int)(c & INT_MASK);
    }
    /*
   ...
   * @throws IOException
   */
  public static void main(@OsUntrusted String @OsUntrusted [] args) throws IOException {
    if (args.length != 1) {
      System.err.println("Usage: JenkinsHash filename");
      System.exit(-1);
    }
    @OsUntrusted
    FileInputStream in = new @OsUntrusted FileInputStream(args[0]);
    @OsUntrusted
    byte @OsUntrusted [] bytes = new @OsUntrusted byte @OsUntrusted [512];
    @OsUntrusted
    int value = 0;
    @OsUntrusted
    JenkinsHash hash = new @OsUntrusted JenkinsHash();
    for (@OsUntrusted int length = in.read(bytes); length > 0 ; length = in.read(bytes)) {

...
public abstract class ProtoUtil {
   ...
   */
  public static @OsUntrusted int readRawVarint32(@OsUntrusted DataInput in) throws IOException {
    @OsUntrusted
      ...
      if ((tmp = in.readByte()) >= 0) {
        ...
        if ((tmp = in.readByte()) >= 0) {
          ...
          if (tmp < 0) {
            ...
            for (@OsUntrusted int i = 0; i < 5; i++) {
              if (in.readByte() >= 0) {
                return result;
              }
            }
            throw new @OsUntrusted IOException("Malformed varint");
          }
        }
      }
    }
   ...
   *
   */
  public static @OsUntrusted IpcConnectionContextProto makeIpcConnectionContext(
      final @OsUntrusted String protocol,
      final @OsUntrusted UserGroupInformation ugi, final @OsUntrusted AuthMethod authMethod) {
    IpcConnectionContextProto.@OsUntrusted Builder result = IpcConnectionContextProto.newBuilder();
    if (protocol != null) {
      result.setProtocol(protocol);
    }
    UserInformationProto.@OsUntrusted Builder ugiProto =  UserInformationProto.newBuilder();
    if (ugi != null) {
      /*
  ...
  public static @OsUntrusted UserGroupInformation getUgi(@OsUntrusted IpcConnectionContextProto context) {
  ...
  public static @OsUntrusted UserGroupInformation getUgi(@OsUntrusted UserInformationProto userInfo) {
    @OsUntrusted
  ...
  static @OsUntrusted RpcKindProto convert(RPC.@OsUntrusted RpcKind kind) {
    switch (kind) {
  ...
  public static RPC.@OsUntrusted RpcKind convert( @OsUntrusted RpcKindProto kind) {
    switch (kind) {
  ...
  public static @OsUntrusted RpcRequestHeaderProto makeRpcRequestHeader(RPC.@OsUntrusted RpcKind rpcKind,
      RpcRequestHeaderProto.@OsUntrusted OperationProto operation, @OsUntrusted int callId,
      @OsUntrusted
      int retryCount, @OsUntrusted byte @OsUntrusted [] uuid) {
    RpcRequestHeaderProto.@OsUntrusted Builder result = RpcRequestHeaderProto.newBuilder();

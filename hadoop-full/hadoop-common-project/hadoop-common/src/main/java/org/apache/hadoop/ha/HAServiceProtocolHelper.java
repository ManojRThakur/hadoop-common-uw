...
public class HAServiceProtocolHelper {
  public static void monitorHealth(@OsUntrusted HAServiceProtocol svc,
      @OsUntrusted
      StateChangeRequestInfo reqInfo)
      throws IOException {
    try {
      svc.monitorHealth();
    } catch (@OsUntrusted RemoteException e) {
      throw e.unwrapRemoteException(HealthCheckFailedException.class);
    }
  }

  public static void transitionToActive(@OsUntrusted HAServiceProtocol svc,
      @OsUntrusted
      StateChangeRequestInfo reqInfo)
      throws IOException {
    try {
  ...
  public static void transitionToStandby(@OsUntrusted HAServiceProtocol svc,
      @OsUntrusted
      StateChangeRequestInfo reqInfo)
      throws IOException {
    try {
